...
// Load the controller
void GazeboRosOdom::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
  // Get the world name.
  ...
  if (!this->link_) {
    ROS_FATAL_NAMED(
        "p3d", "gazebo_ros_p3d plugin error: bodyName: %s does not exist\n",
  ...
  if (!_sdf->HasElement("xyzOffset")) {
    ROS_INFO_NAMED("p3d", "p3d plugin missing <xyzOffset>, defaults to 0s");
    this->offset_.Pos() = ignition::math::Vector3d(0, 0, 0);
  } else
    this->offset_.Pos() =
        _sdf->GetElement("xyzOffset")->Get<ignition::math::Vector3d>();

  if (!_sdf->HasElement("rpyOffset")) {
    ROS_INFO_NAMED("p3d", "p3d plugin missing <rpyOffset>, defaults to 0s");
    this->offset_.Rot() =
        ignition::math::Quaterniond(ignition::math::Vector3d(0, 0, 0));
  } else
    this->offset_.Rot() = ignition::math::Quaterniond(
        _sdf->GetElement("rpyOffset")->Get<ignition::math::Vector3d>());
  ...
  if (!_sdf->HasElement("gaussianBias")) {
    ROS_INFO_NAMED("p3d", "p3d plugin missing <gaussianBias>, defaults to "
                           "0.001, this will cause odom to trackslip");
    this->gaussian_bias_ = 0;
  } else
    this->gaussian_bias_ = _sdf->GetElement("gaussianBias")->Get<double>();

  if (!_sdf->HasElement("updateRate")) {
    ROS_INFO_NAMED("p3d", "p3d plugin missing <updateRate>, defaults to 0.0"
                           " (as fast as possible)");
    this->update_rate_ = 0;
  } else
  ...
  if (this->frame_name_ != "/world" && this->frame_name_ != "world" &&
  ...
  if (this->reference_link_) {
    ROS_DEBUG_NAMED("p3d", "got body %s",
                    this->reference_link_->GetName().c_str());
    this->frame_apos_ = 0;
    this->frame_aeul_ = 0;
#if GAZEBO_MAJOR_VERSION >= 8
  ...
  // start custom queue for p3d
   this->callback_queue_thread_ = boost::thread(
     boost::bind(&GazeboRosOdom::QueueThread, this));
  ...
  if (this->pub_.getNumSubscribers() > 0) {
    ...
    if (tmp_dt != 0) {
      ...
      if (this->topic_name_ != "") {
        ...
        pos_noise_sum.X() += this->GaussianKernel(
            -sign(apos_.X()) * this->gaussian_bias_, this->gaussian_noise_);
        this->pose_msg_.twist.twist.linear.x = vpos.X() ;

        pos_noise_sum.Y() += this->GaussianKernel(
            -sign(apos_.Y()) * this->gaussian_bias_, this->gaussian_noise_);
        this->pose_msg_.twist.twist.linear.y = vpos.Z() ;

        pos_noise_sum.Z() += this->GaussianKernel(
            -sign(apos_.Z()) * this->gaussian_bias_, this->gaussian_noise_);
        this->pose_msg_.twist.twist.linear.x = vpos.Z();

        this->pose_msg_.pose.pose.position.x = pose.Pos().X()+pos_noise_sum.X();
        this->pose_msg_.pose.pose.position.y = pose.Pos().Y()+pos_noise_sum.Y();
        ...
        eul_noise_sum.X() = this->GaussianKernel(
            -sign(aeul_.X()) * this->gaussian_bias_, this->gaussian_noise_);
        this->pose_msg_.twist.twist.angular.x = veul.X() ;

        eul_noise_sum.Y() = this->GaussianKernel(
            -sign(aeul_.Y()) * this->gaussian_bias_, this->gaussian_noise_);
        this->pose_msg_.twist.twist.angular.y = veul.Y() ;

        eul_noise_sum.Z() = this->GaussianKernel(
            -sign(aeul_.Z()) * this->gaussian_bias_, this->gaussian_noise_);
...
double GazeboRosOdom::GaussianKernel(double mu, double sigma) {
...
void GazeboRosOdom::QueueThread() {
